...
type SelectionActionBarProps = {
  /** Called when delete action is triggered with selected IDs */
  onDelete?: (ids: string[]) => void;
  /** Called when export action is triggered with selected IDs */
  onExport?: (ids: string[]) => void;
  /** Custom bulk actions for specific modules */
  bulkActions?: BulkAction[];
};

/**
 * Floating action bar that appears at the bottom when records are selected.
 * Modern Gmail/Linear/Notion style with smooth animations.
 */
        ...
        >
          <div className="flex items-center gap-1 p-2 bg-white/90 dark:bg-gray-900/85 rounded-xl shadow-lg shadow-black/10 dark:shadow-black/25 border border-gray-200/80 dark:border-gray-700/70 backdrop-blur-xl">
            ...
            <button
              ...
              className="flex items-center justify-center w-7 h-7 rounded-lg text-gray-400 border border-gray-200 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100/80 dark:hover:bg-gray-800/70 cursor-pointer bg-transparent transition-colors"
...
const FloatingActionButton = ({
  ...
  const fallbackIcon = (() => {
    ...
    if (normalized.includes("désact") || normalized.includes("desact")) {
      return <IconBan />;
    }
    if (normalized.includes("premium") || normalized.includes("gérer")) {
      return <IconSparkles />;
    }
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <span className="inline-flex items-center justify-center shrink-0">{icon ?? fallbackIcon}</span>
      )}
