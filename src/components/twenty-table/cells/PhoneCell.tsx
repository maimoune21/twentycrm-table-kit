...
export const PhoneCell = ({
  ...
  return (
    ...
    >
      {/* Display */}
      <div
        className={`flex items-center gap-1.5 truncate text-[0.8125rem] text-gray-900 dark:text-gray-100 overflow-hidden text-ellipsis whitespace-nowrap ${isEditMode ? "opacity-50" : ""}`}
      >
        {displayCountry && (
          <span
            className={`fi fi-${displayCountry} w-4 h-3 rounded-sm shadow-sm shrink-0`}
          />
        )}
        {displayContent}
      </div>
